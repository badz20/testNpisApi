...
class ConvertPptToPdf extends Controller
{
    public function convertIntoPdf(Request $request)
    {
        try {
            //code...
        ...
        foreach($rmk_collections as $rmk) {
            ...
            if($rmk->row_status == 1) {
                ...
                $oCell1->getFill()->setFillType(Fill::FILL_GRADIENT_LINEAR)
                    ->setStartColor(new Color('FFFFFF'))
                    ->setEndColor(new Color('FFFFFF'));
                $oCell1->createTextRun($rmk->sdg->nama_sdg)->getFont();
                $oCell1->getActiveParagraph()->getAlignment()->setMarginLeft(10);
                ...
                $oCell1->getFill()->setFillType(Fill::FILL_GRADIENT_LINEAR)
                    ->setStartColor(new Color('FFFFFF'))
                    ->setEndColor(new Color('FFFFFF'));
                $oCell1->createTextRun($rmk->sasaran->Sasaran)->getFont();
                $oCell1->getActiveParagraph()->getAlignment()->setMarginLeft(10);
                ...
                $oCell1->getFill()->setFillType(Fill::FILL_GRADIENT_LINEAR)
                    ->setStartColor(new Color('FFFFFF'))
                    ->setEndColor(new Color('FFFFFF'));
                $oCell1->createTextRun($rmk->indikator->Indikatori)->getFont();
                $oCell1->getActiveParagraph()->getAlignment()->setMarginLeft(10);
            }
    ...
    private function slide2($objPHPPresentation,$project)
    {
        ...
        $oCell2->getActiveParagraph()->getAlignment()
            ...
            ->setHorizontal(Alignment::HORIZONTAL_CENTER);
        if($lokasi) {
            $oCell2->createTextRun($lokasi->parlimen ? $lokasi->parlimen->nama_parlimen : '')->getFont();
        }
        $oCell2->getActiveParagraph()->getAlignment()->setMarginLeft(20);
        ...
        $oCell2->getActiveParagraph()->getAlignment()
            ...
            ->setHorizontal(Alignment::HORIZONTAL_CENTER);
        if($lokasi) {
            $oCell2->createTextRun($lokasi->dun ? $lokasi->dun->nama_dun : '')->getFont();
        }
        $oCell2->getActiveParagraph()->getAlignment()->setMarginLeft(20);
