...
class MasterAgensiController extends Controller
{
    public function list(Request $request)
    {
        try {
            ...
            if($request->has('id')){
                ...
                $data = NamaAgensi::where('id',$request->id)->where('row_status',1)->with(['updatedBy'])->get();
            }else {
                $data = NamaAgensi::where('row_status',1)->with(['updatedBy'])->get();
            }
    ...
    public function store(Request $request)
    {
        try {
            ...
            if(!$validator->fails()) {                   
                if($request->id) {
                    $data = NamaAgensi::where('id', $request->id)->update([
                        'nama_agensi' => $request->name,
                        'dikemaskini_oleh' => $request->user_id,
                        'dikemaskini_pada' => Carbon::now()->format('Y-m-d H:i:s'),
                    ]);
                }else {
                    $data = NamaAgensi::create([                    
                        'row_status' => 1,
                        'nama_agensi' => $request->name,
                        'is_hidden' => 0,                    
    ...
    public function edit($id)
    {
        ...
        try {
            //code...
            $data = NamaAgensi::whereId($id)->with('updatedBy')->first();
